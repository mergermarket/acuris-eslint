    ...
    stages {
        stage ("Test") {
            steps {
                container('cdflow') {
                    sh """
                    docker run -v ${PWD}:/usr/src/app -w /usr/src/app node npm ci ; npm test
                    """
                }
            }
        }
        stage ("Deploy") {
            ...
            steps {
                container('cdflow') {
                    sh """
                    docker run -v ${PWD}:/usr/src/app -w /usr/src/app node cp jenkins.npmrc .npmrc ; npm publish --access public
                    """
                }
            }
        }
